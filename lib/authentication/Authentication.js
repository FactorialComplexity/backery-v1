...
var Authentication = function(application, nconf) {
    ...
    this.getAccessTokensStorage = function() {
    ...
    this.encryptTokens = function(accessToken, refreshToken, clientId) {
        if (clients[clientId] && clients[clientId].encryptionKey) {
            var cipher = crypto.createCipher('aes192', clients[clientId].encryptionKey);
            var encrypted = cipher.update(accessToken + ',' + refreshToken, 'utf8', 'hex');
            encrypted += cipher.final('hex')
            return Backery.Promise.resolve(clientId + ':' + encrypted);
        } else {
            return Backery.Promise.reject(new errors.BackeryUnauthorizedError('Encrypted tokens are not supported for clientId \"' +
                clientId + '\"', 'InvalidTokenError'));
        }
    }
    
    this.decryptTokens = function(encryptedString) {
        var clientIdAndEncryptedTokens = encryptedString.split(':');
        if (clientIdAndEncryptedTokens.length === 2) {
            var clientId = clientIdAndEncryptedTokens[0];
            var encryptedTokens = clientIdAndEncryptedTokens[1];
            
            if (clients[clientId] && clients[clientId].encryptionKey) {
                
                try {
                    var decipher = crypto.createDecipher('aes192', clients[clientId].encryptionKey);
                    var decrypted = decipher.update(encryptedTokens, 'hex', 'utf8');
                    ...
                    return Backery.Promise.reject(new errors.BackeryUnauthorizedError('Encrypted tokens string is invalid', 'InvalidTokenError'));
                }
            } else {
                return Backery.Promise.reject(new errors.BackeryUnauthorizedError('Encrypted tokens are not supported for clientId \"' +
                    clientId + '\"', 'InvalidTokenError'));
