...
var Application = function(nconf, model, Backery) {
    ...
    function isRequestAvailableForUser(request) {
        var isAllow;
        var user = request.user ? request.user.object : undefined;
    ...
    function isUserCanWrite(request) {
        var user = request.user ? request.user.object : undefined;

        var find = _.find(request.entity.getDefinition().fields, function(field){
            var isAllow;
            ...
            if (request.object.changed(field.name)){
                if (user){
                    isAllow = _.find(allowedRoles, function(role) {
                        return role.isUser() || role.isPublic() || user.hasRole(role)
                    });
        ...
        if (request.getValidationError()) {
            ...
            if (request.type != 'CustomEndpoint') {
                ...
                if (isRequestAvailableForUser(request)){
                    if (request.type == 'CreateOrUpdate' && !isUserCanWrite(request)){
                        return new self.Backery.Promise(function(resolve, reject) {
                            reject(new errors.BackeryUnauthorizedError('Forbidden opreation111: ' + _getAccessOperation(request)));
                        });
                    }
