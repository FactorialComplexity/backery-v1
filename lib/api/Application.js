...
var Application = function(nconf, model, Backery) {
    ...
    self.getDisplayName = function() {
        return _model.getDefinition().getDisplayName() || _model.getDefinition().getName();
    }
    ...
    // Google
    var googleTokenChecker = new GoogleTokenChecker(nconf.get('google:clientId'));
        ...
        return new self.Backery.Promise(function(resolve, reject) {
            googleTokenChecker.checkAccessToken(googleAccessToken, function(error, googleUserId) {
                if (!error) {
                    ...
                    reject(new errors.BackeryInvalidParametersError('Invalid google_id_token'));
                }
            });
        });
    }
