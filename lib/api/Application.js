...
var Application = function(nconf, model, Backery) {
    ...
    var googleTokenChecker = new GoogleTokenChecker(nconf.get('google:client_id'),
        nconf.get('twitter:client_secret'));
        
    self.validateGoogleToken = function(twitterAccessToken, twitterAccessSecret) {
        return new self.Backery.Promise(function(resolve, reject) {
            googleTokenChecker.checkAccessToken(twitterAccessToken, twitterAccessSecret, function(error, twitterUserId) {
                if (!error) {
                    resolve(twitterUserId);
                } else {
                    reject(error);
