...
defineSimpleStruct({
    ...
    validate: function(value) {
        if ( !_.isNull(value) && (!_.isObject(value) || !_.isArray(value.coordinates) || value.coordinates.length != 2))
            throw new errors.BackeryTypeError('Value is not an Location');
    },

    toJSON: function(verbose, self) {
        if (_.isNull(self.value)) {
            var value = self.value;
        } else {
            var value = self.value.coordinates;
        }
        return verbose ? { __type: "Location", value: value } : value;
    },
    
    valueFromJSON: function(value) {
        if (!_.isObject(value) && !_.isNull(value))
            new errors.BackeryTypeError('Value is not a Location');
...
var BackeryStructObject = function(entityName, objectId, isRef) {
    ...
    this.getValue = function(key) {
        return _fields[key] ? _fields[key].value : undefined;
    }
    ...
    this.cloned = function() {
        return BackeryStructObject.fromJSON(this.toJSON(true));
    }
...
BackeryStruct.prototype.isObject = function() {
...
BackeryStruct.prototype.isCollection = function() {
...
/// Traverses JSON Object or Array and serializes any occurences of BackeryObject or
/// Backery.Struct into JSON according to verbose setting.
structs.serializeObjectsInJSON = function(json, verbose) {
    if (_.isObject(json) && !_.isArray(json) && !_.isFunction(json) && !_.isDate(json)) {
        if (_.isFunction(json.toStruct)) {
            return json.toStruct({ }).toJSON(verbose);
        } else if (_.isFunction(json.toJSON)) {
            return json.toJSON(verbose);
        } else {
            return _.object(_.map(json, function(value, key) {
                return [key, structs.serializeObjectsInJSON(value, verbose)];
            }));
        }
    } else if (_.isArray(json)) {
        return _.map(json, function(value) {
            return structs.serializeObjectsInJSON(value, verbose);
        });
    } else {
        return json;
    }
