...
defineSimpleStruct({
    ...
    validate: function(value) {
        if ( !_.isObject(value) || !_.isArray(value.coordinates) || value.coordinates.length != 2)
            throw new errors.BackeryTypeError('Value is not an Location');
    },

    toJSON: function(verbose, self) {
        return verbose ? { __type: "Location", value: self.value.coordinates } : self.value.coordinates;
    },
