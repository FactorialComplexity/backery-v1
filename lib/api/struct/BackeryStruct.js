...
defineSimpleStruct({
    ...
    validate: function(value) {
        if (!_.isNull(value) && (!_.isArray(value) || value.length != 2))
            throw new errors.BackeryTypeError('Value is not a Location');
    },

    toJSON: function(verbose, self) {
        return verbose ? { __type: "Location", value: self.value } : self.value;
    },
