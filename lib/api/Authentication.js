...
var Authentication = function(application, nconf) {
    ...
    function _generateRandomToken() {
        crypto.randomBytes(256, function(ex, buffer) {
            if (ex)
                return Backery.Promise.reject(new Error('Failed to generate a random token'));

            return Backery.Promise.resolve(crypto.createHash('sha1').update(buffer).digest('hex'));
        });
    };
    ...
    this.createAccessToken = function(clientId, user) {
        return _generateRandomToken().then(function(accessToken) {
            return database.saveAccessToken(accessToken, clientId,
                nconf.get('oauth:tokensLifetime:accessToken'), user)
            .then(function() {
                return Backery.Promise.resolve({
                    token: accessToken,
                    expiresIn: nconf.get('oauth:tokensLifetime:accessToken')
                });
            });
        });
    }
        ...
        return database.getRefreshToken(refreshToken).then(function(refreshToken) {
            ...
            refreshToken.extendLifetime = function() {
                return _generateRandomToken().then(function(token) {
                    return updateRefreshToken(nconf.get('oauth:tokensLifetime:refreshToken'),
                        token);
                });
            }
        });
    ...
    this.createRefreshToken = function(clientId, user) {
        return _generateRandomToken().then(function(token) {
            return database.saveRefreshToken(token, clientId,
                nconf.get('oauth:tokensLifetime:refreshToken'), user);
        });
    }
    ...
    function getExistingThirdPartyServiceUser(method, params) {
        ...
        if (method == 'facebook') {
            ...
            validator = googleTokenChecker(params.google_id_token);
        }
        
        return validator.then(function(userId) {
            return Backery.Promise.all([
                database.findUserWithThirdPartyServiceUserId(userId),
                userId
            ]);
        });
    }
    ...
    this.authenticateUser = function(method, params) {
        ...
        if (method == 'password') {
            ...
            return Backery.User.query().where(loginField.name, login).findOne().then(function(user) {
                if (user && user.isPasswordCorrect(params.password)) {
                    if (user.object.get('blocked')) {
            ...
            return getExistingThirdPartyServiceUser(methods, params).spread(function(user, thirdPartyUserId) {
                if (user) {
                    return Backery.Promise.resolve(user);
                } else {
