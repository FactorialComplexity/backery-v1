...
var Application = function(nconf, database, Backery) {
    ...
    function isRequestAccessibleByUser(request) {
        ...
        var allowedRoles = _.compact(_.map(request.entity.getDefinition().access.allow[_getAccessOperation(request)], function(roleDefinition) {
    ...
    self.processRequest = function(request, extra) {
        ...
        if (request.getValidationError()) {
            ...
            if (request.type != 'CustomEndpoint') {
                isRequestAccessibleByUser(request).then(function(accessible) {
                    if (accessible) {
