...
module.exports = function(application, config) {
    ...
    if (passwordAuthMethod) {
        ...
        server.post(basePath + '/auth/signup', function(req, res, next) {
            ...
            var createRequest = new requests.CreateRequest(application.Backery.Model.User, req.body, undefined, undefined, application.Backery);
            ...
            if (!req.body.facebook_access_token) {
                ...
                application.validateFacebookToken(req.body.facebook_access_token).then(function(aFacebookUserId) {
                    ...
                    if (user) {
                        ...
                        var createRequest = new requests.CreateRequest(application.Backery.Model.User, {},
    ...
    _.each(modelDefinition.entities, function(entityDefinition) {
        ...
        if (entityDefinition.access.allow['create'].length) {
            server.post(basePath + '/' +  names.toURLCase(entityDefinition.pluralName), function(req, res, next) {
                ...
                processRequest(new requests.CreateRequest(application.Backery.Model[entityDefinition.name], req.body,
                    include, req.user, application.Backery), res, next, 201);
            });
        ...
        if (entityDefinition.access.allow['update'].length) {
            server.put(basePath + '/' +  names.toURLCase(entityDefinition.pluralName) + '/:objectId', function(req, res, next) {
                ...
                processRequest(new requests.UpdateRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, req.body,
                    include, req.user, application.Backery), res, next);
            });
        ...
        if (entityDefinition.access.allow['read'].length) {
            server.get(basePath + '/' +  names.toURLCase(entityDefinition.pluralName) + '/:objectId', function(req, res, next) {
                ...
                processRequest(new requests.ReadRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, include,
                    req.user, application.Backery), res, next);
            });
    ...
    _.each(application.getCustomEndpointsList(), function(endpoint) {
        server[endpoint.method](basePath + endpoint.path, function(req, res, next) {
            ...
            processRequest(new requests.CustomEndpointRequest(endpoint.method, endpoint.path, req.path(),
                req.params, req.body, req.user, application.Backery), res, next);
        });
    });
    
    // HACK: to be removed later, quick solution for batch create of tracks structure
    // var tracksCreate = require('../../../extension/tracksCreate.js');
    // server.post(basePath + '/tracks/create', tracksCreate(application.Backery, respondError, _));
    //
    // var trackUploadAudio = require('../../../extension/trackUploadAudio.js');
    // server.post(basePath + '/tracks/:objectId/media', trackUploadAudio(application.Backery, respondError, _));
    //
