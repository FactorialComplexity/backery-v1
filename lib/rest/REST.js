    ...
    if (googleAuthMethod) {
        oauth.enableGoogle(function(req, callback) {
            if (!req.body.google_access_token) {
                callback(new new errors.BackeryInvalidParametersError('Expected but missing: google_access_token'));
            } else {
                var createRequest = new requests.CreateOrUpdateRequest(application.Backery.Model.User, undefined, {},
                    undefined, undefined, undefined, application.Backery,
                    function(createdUser) {
                ...
                application.validateGoogleToken(req.body.google_access_token).then(function(aGoogleUserId) {
                    if (aGoogleUserId) {
                        ...
                        return application.Backery.Promise.reject(new errors.BackeryInvalidParametersError('Invalid google_access_token'));
                    }
                }).then(function(user) {
                    if (user) {
                        callback(undefined, user);
