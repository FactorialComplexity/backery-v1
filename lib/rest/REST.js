    ...
    _.each(modelDefinition.entities, function(entityDefinition) {
        ...
        if (entityDefinition.access.allow['create'].length) {
            server.post(basePath + '/' +  names.toURLCase(entityDefinition.pluralName), function(req, res, next) {
                ...
                processRequest(new requests.CreateRequest(application.Backery.Model[entityDefinition.name], req.body, application.Backer),
                    res, next, 201);
            });
        ...
        if (entityDefinition.access.allow['read'].length) {
            server.get(basePath + '/' +  names.toURLCase(entityDefinition.pluralName) + '/:objectId', function(req, res, next) {
                ...
                processRequest(new requests.ReadRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, include, application.Backer), res, next);
            });
        ...
        if (entityDefinition.access.allow['update'].length) {
            server.put(basePath + '/' +  names.toURLCase(entityDefinition.pluralName) + '/:objectId', function(req, res, next) {
                ...
                processRequest(new requests.UpdateRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, req.body, application.Backer), res, next);
            });
        ...
        if (entityDefinition.access.allow['delete'].length) {
            server.del(basePath + '/' +  names.toURLCase(entityDefinition.pluralName) + '/:objectId', function(req, res, next) {
                ...
                processRequest(new requests.DeleteRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, application.Backer), res, next);
            });
        ...
        if (entityDefinition.access.allow['query'].length) {
            ...
            var queryHandler = function(req, res, next) {
                ...
                if (req.params.operation && !_.contains(supportedOperations, req.params.operation)) {
                    return respondError(new errors.BackeryInvalidParametersError('Unsupported query operation `' + req.params.operation + '`'), res, next);
                }
                
                processRequest(new requests.QueryRequest(application.Backery.Model[entityDefinition.name], changeCase.camelCase(req.params.operation || 'find'),
