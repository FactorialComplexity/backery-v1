...
module.exports = function(application, config) {
            ...
            ns.run(function(outer) {
                ...
                application.processRequest(request, { clientId: req.clientId }).then(function(responseStruct) {
                    if (request.type != 'CustomEndpoint') {
                        ...
                        if (responseStruct.redirect) {
                            return res.redirect(responseStruct.uri)
                        }
    ...
    if (passwordAuthMethod) {
        ...
        // if reset-password was not overidden in the extenstion code
        if (!application.getSubApplications()['/web/reset-password'] &&
            !application.getSubApplications()['web/reset-password'])
        {
            var resetPassword = express();
            server.use('/web/reset-password', resetPassword);

            resetPassword.set('views', __dirname + '/reset-password/views');
            resetPassword.set('view engine', 'html');
            resetPassword.engine('html', require('hbs').__express);

            resetPassword.use('/css', express.static(__dirname + '/reset-password/css'));
            resetPassword.use('/fonts', express.static(__dirname + '/reset-password/fonts'));
            resetPassword.use('/js', express.static(__dirname + '/reset-password/js'));
