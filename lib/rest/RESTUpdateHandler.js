    ...
    if (entityDefinition.access.allow['update'].length) {
        _.each([true, false], function(isPlural) {
            server.put(basePath + '/' +  router.entityPath(entityDefinition, isPlural) + '/:objectId', function(req, res, next) {
                ...
                var include;
                try {
                    var include = entityDefinition.prepareInclude(router.parseIncludeParameter(req));
                    if (include)
                ...
                router.processRequest(new requests.CreateOrUpdateRequest(application.Backery.Model[entityDefinition.name], req.params.objectId, req.body,
