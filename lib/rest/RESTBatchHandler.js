...
module.exports = function(api, application, router) {
    api.post('/batch', function(req, res, next) {
        console.log('POST %s %j (user: %s)', req.url, req.user ? req.user.id : 'none');
        ...
        function prepareRequest(requestId, json, fileUpload) {
            ...
            var unsupportedParameters = _.difference(json.params, supportedParameters[json.type]);
            ...
            var missingParameters = _.difference(requiredParameters[json.type], params);
            ...
            application.Backery.Promise.mapSeries(requests, function(request) {
                ...
                if (request.type == 'create') {
                    applicationRequest = new application.Request.CreateOrUpdate(application.Backery.Model[request.entityDefinition.name],
                        undefined, request.body, request.file, request.params.include, req.user, application.Backery);
                } else if (request.type == 'update') {
                    applicationRequest = new application.Request.CreateOrUpdate(application.Backery.Model[entityDefinition.name],
                        request.params.objectId, request.body, undefined, request.params.include, req.user, application.Backery);
                } else if (request.type == 'read') {
                    applicationRequest = new application.Request.Read(application.Backery.Model[entityDefinition.name], request.params.objectId,
                    ...
                    applicationRequest = new application.Request.Delete(application.Backery.Model[entityDefinition.name],
