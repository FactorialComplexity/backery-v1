    ...
    var Grant = function(req, res) {
        ...
        this.process = function(user) {
            return self.validateClient(!!user).then(function() {
                ...
                if (!auth.allowsClientIdForUser(self.client.clientId, self.user)) {
                    return Backery.Promise.reject(new errors.BackeryUnauthorizedError('User is not allowed to login with clientId \"' +
                ...
                if (req.body.token_type === 'cookie') {
                    return auth.encryptTokens(self.accessToken.token, self.refreshToken || undefined, self.client.clientId)
        ...
        this.validateClient = function(ignoreMethodAndContentType) {
            ...
            req.clientId = self.client.clientId;
        ...
        this.usePasswordGrant = function() {
            ...
            return auth.authenticateUser('password', credentials, self.client.clientId).then(function(user) {
                if (!user) {
        ...
        this.useRefreshTokenGrant = function() {
            ...
            return auth.getRefreshToken(token).then(function(refreshToken) {
                ...
                return Backery.Model.User.get(refreshToken.user.objectId()).then(function(user) {
                    if (user.get('blocked')) {
