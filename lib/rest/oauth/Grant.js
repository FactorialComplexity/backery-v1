...
module.exports = function grant(auth, config, Backery) {
    var Grant = function(req) {
        ...
        this.process = function(user) {
            return self.validateClient().then(function() {
                // authenticateUser
                if (!user) {
                ...
                var responseData = {
                    ...
                    refresh_token: self.refreshToken ? self.refreshToken.token : undefined,
                    user: grantType != 'refresh_token' ? self.user.toJSON({ verbose: req.backeryShouldRespondVerbose }) :
                        undefined
                };
        ...
        this.usePasswordGrant = funсtion() {
            var credentials = {
        ...
        this.useRefreshTokenGrant = function() {
            ...
            auth.getRefreshToken(token).then(function(refreshToken) {
                if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
                    return Backery.Promise.reject(new errors.BackeryUnauthorizedError('Invalid refresh token',
                        'InvalidTokenError'));
                } else if (refreshToken.expires !== null && refreshToken.expires < self.now) {
        ...
        this.useExtendedGrant = funсtion() {
            return config.extendedGrantHandlers[self.grantType]();
        }
        ...
        this.authenticateUser = function() {
            ...
            return next(new errors.BackeryBadRequestError('Invalid grant_type parameter or parameter missing'));
        }
    }
