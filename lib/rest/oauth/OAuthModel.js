...
OAuthModel.prototype.updateRefreshToken = function(refreshToken, refreshTokenLifetime, callback) {
    console.log('OAuthModel.prototype.updateRefreshToken');
    this.application.getModel().updateRefreshToken(refreshToken, refreshTokenLifetime).then(function(result) {
        callback(undefined);
    }, function(error) {
        callback(error);
    });
