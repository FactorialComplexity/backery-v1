...
function getBearerToken (done) {
  var headerToken = this.req.header('Authorization'),
  ...
  if (methodsUsed > 1) {
    ...
    return done(error('invalidRequest', 'The access token was not found'));
  }
  ...
  if (headerToken) {
    ...
    if (!matches) {
      return done(error('invalidRequest', 'Malformed auth header'));
    }
  ...
  if (postToken) {
    ...
    if (!this.req.is('application/x-www-form-urlencoded')) {
      return done(error('invalidRequest', 'When putting the token in the ' +
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    if (token.expires !== null &&
      ...
      return done(error('tokenExpired',
        'The access token provided has expired.'));
    }
    
    if (token.user && token.user.object.get('blocked')) {
