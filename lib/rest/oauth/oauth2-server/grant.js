 ...
 * @param {Function} next
 */
function Grant (config, req, res, next, user) {
  this.config = config;
  ...
  if (!user){
      ...
      var self = this;
      extractCredentials.apply(this, [function(error) {
          if (!error) {
      ...
      if (!this.client.clientId ||
        ...
        return next(error('invalidClient',
          'Invalid or missing client_id parameter'));
      } else if (!this.client.clientSecret) {
        return next(error('invalidClient', 'Missing client_secret parameter'));
      }
...
function extractCredentials (done) {
  // Only POST via application/x-www-form-urlencoded is acceptableы
  if (this.req.method !== 'POST' ||
      !this.req.is('application/x-www-form-urlencoded')) {
    return done(error('invalidRequest',
  ...
  if (!this.client.clientId ||
      !this.client.clientId.match(this.config.regex.clientId)) {
    return done(error('invalidClient',
      'Invalid or missing client_id parameter'));
  } else if (!this.client.clientSecret) {
    return done(error('invalidClient', 'Missing client_secret parameter'));
  }
...
function checkClient (done) {
  ...
  this.model.getClient(this.client.clientId, this.client.clientSecret,
    ...
    if (!client) {
      return done(error('invalidClient', 'Client credentials are invalid'));
    }
...
function checkGrantType (done) {
  ...
  switch (this.grantType) {
    ...
    case 'encrypted_token':
      return useEncryptedTokenGrant.call(this, done);
...
function useAuthCodeGrant (done) {
  ...
  if (!code) {
    return done(error('invalidRequest', 'No "code" parameter'));
  }
  ...
  this.model.getAuthCode(code, function (err, authCode) {
    ...
    if (!authCode || authCode.clientId !== self.client.clientId) {
      return done(error('invalidGrant', 'Invalid code'));
    } else if (authCode.expires < self.now) {
      return done(error('invalidGrant', 'Code has expired'));
    }
...
function usePasswordGrant (done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(error('serverError', false, err));
    if (!user) {
        return done(error('invalidCredentials', 'User credentials are invalid'));
    }
...
function useEncryptedTokenGrant(done){
  ...
  this.req.body.refresh_token = tokens.split('_')[1];

  useRefreshTokenGrant.call(this, done);
}

function useRefreshTokenGrant (done) {
  var token = this.req.body.refresh_token;
  if (!token) {
    return done(error('invalidRequest', 'No "refresh_token" parameter'));
  }
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
      return done(error('invalidToken', 'Invalid refresh token'));
    } else if (refreshToken.expires !== null &&
        refreshToken.expires < self.now) {
      return done(error('tokenExpired', 'Refresh token has expired'));
    }
    ...
    if (refreshToken.user.object.get('blocked')) {
      return done(error('userIsBlocked', 'User is blocked'));
    }

    self.user = refreshToken.user || { id: refreshToken.userId };
    
    self.model.updateRefreshToken(token, self.config.refreshTokenLifetime, function(err){
      if (err) return done(error('serverError', false, err));
      done();
    });
...
function useClientCredentialsGrant (done) {
  ...
  return this.model.getUserFromClient(clientId, clientSecret,
    ...
    if (err) return done(error('serverError', false, err));
    if (!user) {
      return done(error('invalidGrant', 'Client credentials are invalid'));
    }
...
function useExtendedGrant (done) {
  ...
  this.model.extendedGrant(this.grantType, this.req,
    ...
    if (err) {
      return done(error(err.error || 'serverError',
    ...
    if (!supported) {
      ...
      return done(error('invalidRequest', 'Invalid request.'));
    }
...
function sendResponse (done) {
  ...
  if (!this.req.body.cookie){
    ...
    var key = this.model.application.getCookiePrivateKey();
    encodedKey = aes.encText([this.accessToken, this.refreshToken].join('_'), key);
    this.res.cookie('cookie-bearer', encodedKey, {
