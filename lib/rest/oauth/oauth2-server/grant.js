...
function Grant (config, req, res, next, user) {
  ...
  if (!user){
      runner([extractCredentials], this);
      if (this.grantType == 'refresh_token'){ // проверить что тут с ошибками
        fns.splice(fns.indexOf(generateRefreshToken), 1);
        fns.splice(fns.indexOf(saveRefreshToken), 1);
        runner([
          checkClient,
          checkGrantTypeAllowed,
          checkGrantType,
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
      return done(error('invalidGrant', 'Invalid refresh token'));
    } else if (refreshToken.expires !== null &&
        refreshToken.expires < self.now) {
      return done(error('invalidGrant', 'Refresh token has expired'));
    }
...
function sendResponse (done) {
  ...
  if (!this.req.body.with_cookie){
    ...
    this.res.setCookie('cookie-bearer', encodedKey);
  }
