...
SequelizeModel.prototype.define = function(definition, databaseURI, databaseOptions, Backery) {
    ...
    self.transformAttributes = function(entityDefinition, params) {
        ...
        return _.object([[
            ...
            _.map(keys, function(key) {
                if (key == '$nearby') {
                    return [Sequelize.literal('( 6371 * acos( cos( radians('+params[key].lat+') ) * cos( radians(  X(  `location` ) ) ) * cos( radians( Y(  `location` ) ) - radians('+params[key].lon+') ) + sin( radians('+params[key].lat+') ) * sin( radians( X(  `location` ) ) ) ) )'), 'distance']
                }
            })
        ]]);
    }
    ...
    self.transformHaving = function(entityDefinition, where) {
        ...
        return _.map(keys, function(key) {
            if (key == '$nearby') {
                return _.object([['distance', _.object([['$lte', where[key].distance]])]])
            }
        });
    }
