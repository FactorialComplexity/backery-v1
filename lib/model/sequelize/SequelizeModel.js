...
SequelizeModel.prototype._object = function(sequelizeInstance, entityDefinition) {
    ...
    return new BackeryObject(new SequelizeObjectImpl(sequelizeInstance, entityDefinition, this));
}
...
SequelizeModel.prototype.authUser = function(method, params) {
    ...
    if (!methodDefinition) {
        throw new errors.BackeryConsistencyError('Login/password based authentication method is not supported by the model');
    }
    ...
    if (method == 'password') {
        ...
        if (usernameString.indexOf(':') != -1) {
            ...
            if (!loginField) {
                throw new errors.BackeryConsistencyError('Invalid login field: `' + split[0] + '`');
            }
