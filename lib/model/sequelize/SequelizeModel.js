...
SequelizeModel.prototype.define = function(definition, databaseURI, databaseOptions, Backery) {
    ...
    _.each(definition.entities, function(entity, name) {
        ...
        if (entity.isUserEntity()) {
            dbFields['disabledAt'] = Sequelize.DATE;
            ...
            self._sequelizeModels.User.findOne({ where: where }).then(function(sequelizeInstance) {
                ...
                if (user) {
                    ...
                    if (user.getDisabledAt()) {
                        reject(new errors.BackeryUnauthorizedError(
                            'Authentification invalid: user disabled!'));
                    } else if (user.isPasswordCorrect(params.password)) {
                        resolve(user);
                    } else {
                        reject(new errors.BackeryInvalidParametersError(
                            'Authentification invalid: wrong password!'));
                    }
                } else {
                    reject(new errors.BackeryInvalidParametersError(
                      'Authentification invalid: wrong ' + loginField.name +'!'));
                }
            }, function(error) {
                  reject(new errors.BackeryDatabaseError(error));
            });
