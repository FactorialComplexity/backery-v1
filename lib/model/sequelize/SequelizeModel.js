            ...
            if (op == '$and' || op == '$or') {
                ...
                if (databaseOptions.caseSensitiveCollation) {
                    return Sequelize.literal('`' + entityDefinition.name + '`.`' + _.keys(param)[0] +
                        '` LIKE \'%' + param[_.keys(param)[0]] + '%\' COLLATE \'' + databaseOptions.caseSensitiveCollation + '\'');
                } else {
                    return _.object([[
                        _.keys(param)[0],
                        _.object([[
                ...
                if (databaseOptions.caseInsensitiveCollation) {
                    return Sequelize.literal('`' + entityDefinition.name + '`.`' + _.keys(param)[0] +
                        '` LIKE \'%' + param[_.keys(param)[0]] + '%\' COLLATE \'' + databaseOptions.caseInsensitiveCollation + '\'');
                } else {
                    return _.object([[
                        _.keys(param)[0],
                        _.object([[
        ...
        _.each(entity.fields, function(field) {
            ...
            if (field.type.value == TypeDefinition.String) {
                if (_.contains(loginFields, field)) {
                    dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = {
                        type: Sequelize.STRING(40),
                        unique: true
                    };
                } else {
                    dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.TEXT, defaultValue: null };
                }
            } else if (field.type.value == TypeDefinition.Integer) {
                dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.INTEGER, defaultValue: null };
            } else if (field.type.value == TypeDefinition.Number) {
                dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.DOUBLE, defaultValue: null };
            } else if (field.type.value == TypeDefinition.Boolean) {
                dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.BOOLEAN, defaultValue: null };
            } else if (field.type.value == TypeDefinition.Date) {
                dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.DATE, defaultValue: null };
