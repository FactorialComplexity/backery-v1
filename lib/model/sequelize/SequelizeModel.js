    ...
    self.transformWhere = function(entityDefinition, where) {
        function mapCondition(condition) {
            ...
            if (key == '$and' || key == '$or') {
                ...
                var field = _.keys(value)[0];
                if (entityDefinition.fields[field].type.value == TypeDefinition.Relation_One) {
                    return _.object([SequelizeNameMapper.sequelizeAssociationForeignKey(entityDefinition.fields[field])], [value[field].objectId()]);
                }else{
                    return value;
                }
            } else if (key == '$ne' ||
                ...
                if (databaseOptions.caseSensitiveCollation) {
                    return Sequelize.literal('`' + entityDefinition.name + '`.`' + _.keys(value)[0] +
                        '` LIKE \'%' + value[_.keys(value)[0]] + '%\' COLLATE \'' + databaseOptions.caseSensitiveCollation + '\'');
                } else {
                    return _.object([[
                        _.keys(value)[0],
                        _.object([[
            ...
            }  else if (key == '$containsCI') {
                if (databaseOptions.caseInsensitiveCollation) {
                    return Sequelize.literal('`' + entityDefinition.name + '`.`' + _.keys(value)[0] +
                        '` LIKE \'%' + value[_.keys(value)[0]] + '%\' COLLATE \'' + databaseOptions.caseInsensitiveCollation + '\'');
                } else {
                    return _.object([[
                        _.keys(value)[0],
                        _.object([[
    ...
    _.each(definition.entities, function(entity, name) {
        ...
        if (entity.isUserEntity()) {
            ...
            _.each(entity.modelDefinition.roles, function(role){
                if (!role.isVirtual()) {
                    dbFields['__role' + role.name] = Sequelize.BOOLEAN;
                }
            });
        ...
        _.each(entity.fields, function(field) {
            ...
            if (field.type.value == TypeDefinition.String) {
                if (_.contains(loginFields, field)) {
                    ...
                    dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.TEXT };
                }
                ...
                dbFields[SequelizeNameMapper.sequelizeFieldName(field)] = { type: Sequelize.DATE };
