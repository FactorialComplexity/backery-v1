        ...
        function mapAttributes(params) {
            return _.object([[
                ...
                _.map(params, function(pair) {
                    var key = _.keys(pair)[0];
                    if (key == '$nearby') {
                        return [Sequelize.literal('( 6371 * acos( cos( radians('+pair[key].lat+') ) * cos( radians(  X(  `location` ) ) ) * cos( radians( Y(  `location` ) ) - radians('+pair[key].lon+') ) + sin( radians('+pair[key].lat+') ) * sin( radians( X(  `location` ) ) ) ) )'), 'distance']
                    }
                }).filter(function(x) {
                    return !_.isUndefined(x);
                })
...
SequelizeModel.prototype.findUserWithPasswordRecoveryToken = function(token) {
    ...
    return new self.Backery.Promise(function(resolve, reject) {
        self._sequelizeModels.User.findOne({ where: { __passwordRecoveryToken: token } }).then(function(user) {
            if (user) {
                resolve(self.createBackeryObject(user, 'User'));
            } else {
                resolve();
            }
        }, function(error) {
...
SequelizeModel.prototype.authUser = function(method, params) {
    ...
    if (method == 'password') {
        ...
        return new self.Backery.Promise(function(resolve, reject) {
            self._sequelizeModels.User.findOne({ where: { '__twitterUserId': params.twitterUserId } }).then(function(sequelizeInstance) {
                resolve(self.createBackeryObject(sequelizeInstance, 'User'));
            }, function(error) {
                reject(new errors.BackeryDatabaseError(error));
            });
