...
var SequelizeQueryImpl = function(entityDefinition, sequelizeEntityModel, sequelizeModel) {
    ...
    this._prepareQuery = function(query, defaultOffset, defaultLimit) {
        ...
        var self = that;
        var data = {
            where: self.sequelizeModel.transformWhere(entityDefinition, query.getWhere()),
            include: self.sequelizeModel.transformInclude(entityDefinition, query.getInclude()),
            order: self.sequelizeModel.transformSort(entityDefinition, query.getSort()),
...
SequelizeQueryImpl.prototype.find = function(query) {
    ...
    return self.promiseWrap(self.sequelizeEntityModel.findAll(self._prepareQuery(query, 0, 100)), function(sequelizeInstances) {
        return _.map(sequelizeInstances, function(sequelizeInstance) {
            return self.sequelizeModel.createBackeryObject(new SequelizeObjectImpl(sequelizeInstance, self.definition, self.sequelizeModel));
        });
    });
...
SequelizeQueryImpl.prototype.count = function(query) {
    return this.promiseWrap(this.sequelizeEntityModel.count(this._prepareQuery(query)));
}
