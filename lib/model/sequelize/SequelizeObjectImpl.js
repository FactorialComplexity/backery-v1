...
var SequelizeObjectImpl = function(sequelizeInstance, entityDefinition, context, include) {
    ...
    self.relationsMany = _.object(_.compact(_.map(self.entityDefinition.fields, function(fieldDefinition) {
        if (fieldDefinition.type.isRelationMany()) {
            return [fieldDefinition.name, new BackeryRelation(new SequelizeRelationImpl(self, fieldDefinition,
                _.extend(self.context, { SequelizeObjectImpl: SequelizeObjectImpl }), include))];
        }
    })));
}
