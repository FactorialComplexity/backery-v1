...
SequelizeObjectImpl.prototype.User_getRoles = function() {
    var self = this;
    return _.map(this.entityDefinition.modelDefinition.roles, function(role) {
        if (!role.isVirtual()) {
            var rolePresence = self.sequelizeInstance.get('__role' + role.name);
            if (rolePresence){
                return role.name;
            }
        }
...
SequelizeObjectImpl.prototype.User_getRole = function(role) {
    return this.sequelizeInstance.get('__role' + role);
