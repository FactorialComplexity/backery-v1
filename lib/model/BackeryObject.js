...
var BackeryObject = function(impl, Backery, databaseHooks) {
    ...
    this.toStruct = function(options) {
        ...
        var user = (options && options.user) ? options.user : undefined;
        
        function canRead(field) {
            var isAllow;
            console.log(field.access.allow.read)
            var allowedRoles = _.map(field.access.allow.read, function(roleDefinition){
                console.log(roleDefinition.name, Backery.Model.Roles[roleDefinition.name])
                
                return Backery.Model.Roles[roleDefinition.name];
            });
            
            if (allowedRoles) {
                if (user || (options.access && options.access.isUser)) {
                    isAllow = !!_.find(allowedRoles, function(role) {
                        return role.isUser() || role.isPublic() || (user && user.hasRole(role));
                    });
                } else {
                    isAllow = !!_.find(allowedRoles, function(role) {
