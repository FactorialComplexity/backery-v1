...
var BackeryFile = function(impl, Backery) {
    ...
    this.destroy = function() {
        return self.fetchIfNeeded().then(function(){
            var storage = impl.File_getStorage();
            
            return _super.destroy().then(function() {
    ...
    this.save = function() {
        ...
        if (self.isNew()) {
            return manager.upload(_body.path, mediaUUID + (_body.preserveName ?
                ('/' + _body.name) : path.extname(_body.name)),
                { content_type: _body.contentType })
