...
var EntityDefinition = function(data, modelDefinition, ignorePredefined, defaultAccessData) {
    ...
    this.access = new EntityAccessDefinition(_.extend(defaultAccessData, data.access));
}
...
EntityDefinition.Predefined = {
    ...
    Installation: {
        name: 'Installation',
        fields: [
            { name: "platform", type: { name: "String", limit: 20 } },
            { name: "apnsToken", type: { name: "String", limit: 64 }, unique: true },
            { name: "gcmToken", type: { name: "String", limit: 100 }, unique: true },
...
EntityDefinition.prototype.prepareValues = function(values, Backery) {
    ...
    if (invalidFields.length) {
        throw new errors.BackeryInvalidParametersError('Invalid fields in values for entity `' + self.name + '`: ' +
...
EntityDefinition.prototype.prepareSort = function(sort) {
    ...
    sort = _.map(sort, function(pair) {
        ...
        // Check key for validity
        var field = self.fields[key];
        if (!field && key != "id" && key != "random" && key != "createdAt" && key != "updatedAt") {
