...
EntityDefinition.prototype.prepareValues = function(values, Backery) {
    ...
    function compositeWithDeserializedObjects(composite) {
        if (_.isArray(composite)) {
            return arrayWithDeserializedObjects(composite);
        } else if (_.isObject(composite)) {
            return dictionaryWithDeserializedObjects(composite);
        }
    }
    ...
    _.each(values, function(value, key) {
        if (self.fields[key]) {
            if (self.fields[key].type.isString()) {
                ...
                } else if (!isCompositeValueValid(value)) {
