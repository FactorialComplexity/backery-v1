...
EntityDefinition.prototype.prepareWhere = function(where, entities) {
    ...
    function parseWhere(where){
        return _.object( _.map(_(where).pairs(), function(pair){
            if (pair[0] == '$or' || pair[0] == '$and'){
                if (!_.isArray(pair[1]))
                        throw new errors.BackeryInvalidParametersError('Invalid $or of $and object');
                return [ pair[0], _.map(pair[1], function(pair){
                    return parseWhere(pair);
                }) ];
            }else if (pair[0] == '$eq' || pair[0] == '$ne'){
                if (!_.isObject(pair[1])){
                    throw new errors.BackeryInvalidParametersError('Invalid $eq of $ne object');
                }
                var value = _.values(pair[1])[0];
                var key = _.keys(pair[1])[0];
                if (self.fields[key].type.isRelationOne()){
                    var entity = entities[self.fields[key].type.relatedEntity.name];
                    return [pair[0], _.object([key],[entity.ref(value)])];
                }else{
                    return [pair[0], _.object([key],[value])];
                }
            }else if (pair[0] == '$gte' || pair[0] == '$gt' || pair[0] == '$lt' || pair[0] == '$lte'){
                var value = _.values(pair[1])[0];
                // if(!_.isNumber( value )){
                //     throw new errors.BackeryInvalidParametersError('Invalid numeral parameter');
                // }
                if(!self.fields[_.keys(pair[1])[0]].type.isDate()){
                    value = new Date(value);
                }
                return [pair[0], _.object([_.keys(pair[1])[0]], [value])];
            }else if (pair[0] == '$in' || pair[0] == '$notIn'){
                return [pair[0], pair[1]];
            }else if (pair[0] == '$contains' || pair[0] == '$containsCI'){
                return [pair[0], pair[1]];
            } 
        }) )
    }
