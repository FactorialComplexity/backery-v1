...
var BackeryRelationOperation = function(data, relatedEntity) {
    ...
    if (!_.isObject(data) || hasNotAllowedKeys(data, ['__type', 'action', 'objects'])) {
        throw new errors.BackeryInvalidParametersError('RelationOperation data object contains unsupported keys');
    }
    ...
    if (!_.contains(['set', 'add', 'remove'], data['action'])) {
        throw new errors.BackeryInvalidParametersError('RelationOperation data object has incorrect type');
    }
    ...
    if (_.detect(data['objects'], function(objectId) {
        ...
        throw new errors.BackeryInvalidParametersError('RelationOperation data object has incorrect objects');
    } else {
