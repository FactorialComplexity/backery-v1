...
var BackeryRelationOperation = function(set, add, remove, relatedEntity) {
    ...
    this.getChanges = function() {
        return {
            add: _set ? undefined : _add,
            remove: _set ? undefined : _remove,
            set: _set
        };
    }
...
BackeryRelationOperation.Set = function(objects, relatedEntity) {
    if (!_.isUndefined(objects) && !_.isArray(objects)) {
        objects = [objects];
    }
...
BackeryRelationOperation.AddRemove = function(add, remove, relatedEntity) {
    if (!_.isUndefined(add) && !_.isArray(add)) {
        add = [add];
    }
    
    if (!_.isUndefined(remove) && !_.isArray(remove)) {
        remove = [remove];
    }
